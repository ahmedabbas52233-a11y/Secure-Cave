...
function LoadingScreen() {
  const { C } = useTheme();
  return (
    <div style={{ minHeight: "100vh", background: C.bgVoid, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
      <div style={{ width: 40, height: 40, border: `2px solid ${C.bgElevated}`, borderTop: `2px solid ${C.ember}`, borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
      <p style={{ color: C.textDark, fontFamily: "'Syne', sans-serif", fontSize: 12, letterSpacing: "0.1em" }}>ENTERING CAVE…</p>
    </div>
  );
}
...
function InnerApp() {
  ...
  // Auto-lock: read from localStorage, 0 = disabled
  const [autoLockMs, setAutoLockMs] = useState(
    () => parseInt(localStorage.getItem("sc_autolock") || "0", 10)
  );
  ...
  // ── Auto-lock (only active when vault is unlocked) ────────────────
  const handleLock = useCallback(() => {
    setUnlocked(false); setMasterKey(null);
    setActiveView("vault"); setModal(null);
  ...
  const openAdd = () => {
    ...
    setEditEntry(null); setModal("add");
  };
  const openEdit = async (entry) => {
    ...
    setEditEntry(entry); setModal("edit");
  };
  ...
  const handleSave = async () => {
    try {
      if (modal === "add") { await vault.addEntry(form); showNotif("Secret carved into stone 🪨"); }
      else { await vault.updateEntry(editEntry._id || editEntry.id, form); showNotif("Tablet updated 🗿"); }
      setModal(null); setEditEntry(null);
    } catch (e) { showNotif(e.message, "error"); }
  };
  const handleDelete = async () => {
    try { await vault.deleteEntry(deleteId); showNotif("Entry removed from cave", "warning"); }
    catch (e) { showNotif(e.message, "error"); }
    ...
    Papa.parse(file, {
      ...
      complete: async ({ data }) => {
        ...
        for (const row of data) {
          ...
          try { await vault.addEntry({ site: row.site, username: row.username || "", password: row.password, category: row.category || "Other" }); count++; }
          catch {}
        }
        showNotif(`Imported ${count} entries 📥`);
      },
      error: () => setImportError("Failed to parse CSV"),
    });
  };
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", background: C.bgVoid }}>
      <Sidebar
        activeView={activeView}
        setActiveView={setActiveView}
        vaultCount={vault.vault.length}
        onLock={handleLock}
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        {/* Top bar */}
        <div className="main-topbar" style={{ padding: "18px 28px 14px", borderBottom: `1px solid ${C.borderDim}`, background: C.bgVoid, flexShrink: 0, display: "flex", alignItems: "flex-start", gap: 12 }}>
          {/* Hamburger — hidden on desktop via CSS */}
          <button className="hamburger-btn icon-btn" onClick={() => setSidebarOpen(true)} style={{ marginTop: 2, flexShrink: 0 }}>
            <Menu size={16} />
          </button>
          <div>
            <h2 style={{ fontFamily: "'Cinzel', serif", color: C.textBone, fontSize: 17, fontWeight: 700, margin: 0 }}>{VIEW_TITLE[activeView]}</h2>
            <p style={{ color: C.textDark, fontSize: 11, marginTop: 3, fontFamily: "'Syne', sans-serif", letterSpacing: "0.05em" }}>{VIEW_SUB[activeView]}</p>
          </div>
        </div>

        {/* Content */}
        <div className="main-content" style={{ flex: 1, overflowY: "auto", paddingTop: 20 }}>
          {activeView === "vault" && (
            <VaultView
              vault={vault.vault} decryptedPws={vault.decryptedPws}
              visiblePws={vault.visiblePws} loading={vault.loading}
          ...
          {activeView === "settings" && (
            <SettingsView
              ...
              importError={importError} vaultCount={vault.vault.length}
              autoLockMs={autoLockMs} onAutoLockChange={updateAutoLock}
            />
          )}
        </div>
      </div>
